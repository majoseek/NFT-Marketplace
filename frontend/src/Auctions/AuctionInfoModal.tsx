...
const AuctionInfoModal = ({ auctionId, visible, setVisible }: Props) => {
  ...
  const handlePlaceBid = async () => {
    ...
    (auctionContract.methods.placeBid as any)(auctionId, bidAmountInWei)
      .send({ from: userAccountAddress, gas: 3000000, value: bidAmountInWei })
  ...
  return (
    <Modal
      ...
      onOk={handlePlaceBid}
      okButtonProps={{ disabled: bidAmount === undefined || bidAmount === 0 }}
    >
        ...
        {auctionInfo ? (
          isBidding ? (
            ...
            <>
              ...
              <span>
                Current bid:
                {Math.max(...auctionInfo.bids.map((bid) => bid.amount))}
              </span>
              <span>Expiration time: {auctionInfo.expiryTime}</span>
              <Input placeholder="Enter your bid" onChange={handleChangeBid} />
            </>
          )
        ) : (
          <Spin />
